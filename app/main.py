...
# Simple persistence using SQLite-like JSON files (portable on Vercel/local)
# We use filesystem JSON for portability; for true production, plug in a DB.
...
# --- Lightweight TF-IDF semantic search (no heavy deps) ---
import math
import re
...
class LocalSearchIndex:
    ...
    def _cosine(vec: Dict[str, float], tf_doc: Counter[str], idf: Dict[str, float], norm_doc: float) -> float:
        # Compute dot product between query vec and doc vec lazily
        dot = 0.0
...
async def save_memory(req: SaveMemoryRequest):
    ...
    # Rebuild index lazily next query
    return {"ok": True}
...
async def search(req: SearchRequest):
    SEARCH_INDEX.build()
    results = SEARCH_INDEX.query(req.query, top_k=req.top_k)
...
async def agent_step(req: AgentStepRequest):
    ...
    # If confirmation required and not yet approved for this turn, return proposal only
    if req.require_confirmation:
        # Ask model to draft a plan (no execution)
        try:
    ...
    # Otherwise, execute with model
    try:
        reply = await openrouter_chat(
            messages=[
                {"role": "system", "content": "Execute the plan. Keep responses concise and actionable."},
                *session["messages"],
            ],
            model=session.get("model"),
        )
...
# Static index.html for local run convenience
@app.get("/")
async def root_html():
